...
start([ClusterSpec,_Arg2])->
    ...
    ok=stop_all_pods(ClusterSpec),
    io:format("local apps running OK !!! ~p~n",[{?MODULE,?LINE,?FUNCTION_NAME}]),

    {ok,ActiveParents}=start_parents(ClusterSpec),
						%    io:format("ActiveParents OK !!! ~p~n",[{ActiveParents,?MODULE,?LINE,?FUNCTION_NAME}]),
    {ok,ActivePods}=start_pods(ClusterSpec),
 %   io:format("ActivePods  OK !!! ~p~n",[{lists:sort(ActivePods),?MODULE,?LINE,?FUNCTION_NAME}]),
    io:format("All pods are runing OK !!! ~p~n",[{?MODULE,?LINE,?FUNCTION_NAME}]),
    
    ok=start_common_sd(ActivePods),
    io:format("Common and sd running on all pods  OK !!! ~p~n",[{?MODULE,?LINE,?FUNCTION_NAME}]),

    ok=start_nodelog(ClusterSpec),
    io:format("Nodelog is running on   OK !!! ~p~n",[{sd:get_node(nodelog),?MODULE,?LINE,?FUNCTION_NAME}]),
    
    ok=start_etcd(ClusterSpec),
    io:format("Etcd is running on   OK !!! ~p~n",[{sd:get_node(etcd),?MODULE,?LINE,?FUNCTION_NAME}]),
    
%    ok=start_parents(),
%    ok=start_infra_appls(ClusterSpec),
%    sd:call(control,control,start_orchistrate,[],5000),
...
%    ok=check_appl_status(),
 ...
 %   ok=create_initial_node(),
  ...
  % Creat parent 
    ...
    WhichApplications=[{Node,rpc:call(Node,application,which_applications,[],5000)}||Node<-nodes()],
